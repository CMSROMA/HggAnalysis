...
//     mc_tth_fill[i]->setCic(cic);
  ...
  vardata->Add((TH1D*)data_fill.File("/tmp/delre/data.root")->Get("massgg")); 
  ...
  vardatacs->Add((TH1D*)datacs_fill.File("/tmp/delre/datacs.root")->Get("massgg")); 
  ...
  for (int i=0; i<9; i++){ 
	...
	// 	if(i==7) mc_fill[i]->Writetxt(name);
      ...
      var_mc[i]->Add((TH1D*)mc_fill[i]->File(name)->Get("massgg"));
  ...
  for (int i=0; i<7; i++){ 
    ...
    var_gluglu[i]->Add((TH1D*)mc_gluglu_fill[i]->File(name)->Get("massgg"));
    ...
    var_vbf[i]->Add((TH1D*)mc_vbf_fill[i]->File(name)->Get("massgg"));
    ...
    var_wzh[i]->Add((TH1D*)mc_wzh_fill[i]->File(name)->Get("massgg"));
...
//     std::cout << "Selected events on mc2011 tth " << h_masses[i] << " " << var_tth[i]->GetEntries() << std::endl;
  ...
  if (variable == "massgg")
    {  
      ...
      integralhiggs = mydoublegau->Integral(116.,124.)/binsize;
      integralbkg = mypowlaw->Integral(116.,124.)/binsize;
